...
# 节点保存的value定义如下：
class Value:
    def __init__(self, path, torrent_flag=False, hash=None):
        """
        @param
        hash:分片/种子的hash值
        path:文件/种子存储路径
...
class ValueGenerator:
    ...
    def split_single_file(self, storage):
        ...
        if size <= chunksize:
            ...
            value = Value(filename, False, chunk_hash.hex())
            storage[chunk_hash] = value
            value_list.append(value)
            with open(filename, 'wb') as fileptr:
                fileptr.write(chunk)
            fileptr.close()
            return value_list, chunk_hash
        ...
        if os.path.exists(target):
            ...
            for hash in hash_list:
                ...
                value_list.append(storage[hash])
            # 保存种子信息到storage
            key = str_to_bytes(file_hash)
            storage[key] = Value(os.path.join(target, file_hash[:10]), True, file_hash)
            value_list.append(storage[key])
            return value_list, file_hash
